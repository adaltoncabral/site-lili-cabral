...
const Carousel = ({ items, autoPlayInterval = 5000, renderItem, className = '' }) => {
  ...
  return (
    <div className={`relative ${className}`}>
      {items.map((item, index) => (
        <div key={index} className={`transition-opacity duration-1000 ${index === currentIndex ? 'opacity-100' : 'opacity-0 absolute inset-0'}`}>
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-rose-50">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-pink-800 mb-4" style={{ fontFamily: 'Modish, serif' }}>
              Nossas Categorias
            </h2>
              ...
              <div key={index} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group">
                <div className="p-6 text-center">
                  <div className="w-16 h-16 mx-auto mb-4 bg-pink-100 rounded-full flex items-center justify-center group-hover:bg-pink-200 transition-colors duration-300">
                    <Image src={category.icon} alt={category.alt} width={32} height={32} />
                  </div>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-pink-500 hover:bg-pink-600 text-white px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 inline-flex items-center space-x-2"
                  >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative">
            <div className="bg-pink-50 rounded-2xl p-8 text-center">
              <StarRating rating={APP_DATA.testimonials[currentTestimonial].rating} />
              <p className="text-lg text-gray-700 mb-6 italic">
                "{APP_DATA.testimonials[currentTestimonial].text}"
              </p>
              <h4 className="text-xl font-semibold text-pink-800">
                {APP_DATA.testimonials[currentTestimonial].name}
              </h4>
            </div>

            <div className="flex justify-center mt-6 space-x-2">
              {APP_DATA.testimonials.map((_, index) => (
                <button
                  key={index}
                  onClick={() => setCurrentTestimonial(index)}
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentTestimonial ? 'bg-pink-500' : 'bg-pink-200'}`}
                />
              ))}
            </div>
          </div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-pink-500 to-rose-500">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4" style={{ fontFamily: 'Modish, serif' }}>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="flex items-center space-x-2 mb-4 md:mb-0">
              <HeartIcon className="h-6 w-6 text-pink-300" />
